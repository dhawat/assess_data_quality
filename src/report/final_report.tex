...
It defines the range of admissible values within a column e.g., a specific data type, an interval for numerical columns, or a set of values for categorical ones.
...
\subsubsection{Order relation between columns} % (fold)
...
\subsubsection{Outlier detection with machine learning} % (fold)
...
%     \label{fig:lof}
% \end{figure}
The local outlier factor (LOF) is an unsupervised machine learning outlier detection method that attempts to harness the idea of nearest neighbors for outlier detection, i.e., identify local outliers.
