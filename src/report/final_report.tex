...
\title{\Large Automatic data quality assessment \\
\small Report for AMIES challengedoc, Wasserstein-blind group}
\author{ Mariem Abaach, Mehdi Boussaa, Diala Hawat}
\usepackage{fancyhdr}
%align minpages fig
%\pagestyle{fancy}
%\makeatletter
...
At the start of the challenge our correspondant at Foyer shared with us a dataset comprised of 100000 rows and 54 columns of sales of agricultural machinery.
The data gather different types of columns like numerical non-discrete e.g., \texttt{price}, numerical category (or discrete) e.g., \texttt{ModelId} and nominal columns e.g., \texttt{state}.
Not all the columns were directly usable such as \texttt{saledate} which was in a non classical format.
...
\begin{figure}[H]
    ...
    \begin{minipage}[t]{.35\textwidth}
        \centering
        \includegraphics[width=0.9\linewidth]{picture/exp_duplicated.png}
        \caption{Sample from the detected duplicated rows}
    ...
    \begin{minipage}[t]{.55\textwidth}
        ...
        \vspace*{-0.39cm}
        \caption{Two duplicated rows from the data set}
        \label{fig:exp_duplicate_2}
    \end{minipage}
\end{figure}
The first pass start by the redundency test described in Section \ref{sub:Redundancy} to find duplicated rows.
The corresponding algorithm returns the index of the duplicated rows as we can see in Figure \ref{fig:exp_duplicate}.
For example, the first row of Figure \ref{fig:exp_duplicate_2}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.2\linewidth]{picture/exp_typo.png}
    \caption{Sample from the detected typographical errors}
    \label{fig:exp_typo}
\end{figure}
Figure \ref{fig:exp_typo}, shows a sample from the detected typographical errors in the column \texttt{UsageBand}.
Multiple instances of the values 'HIGH' and 'high' were detected as errors.
Our correspondant confirm inducing these errors since the company was interrested in finding such typographical error frequently found in there database.
The correct spelling is 'High'.
Both the affinity propagation and Markov clustering methods described in Section \ref{sub:consistency} successfully detect these errors.
However, when the database gets bigger and more words appear in each cluster, the affinity propagation clustering fails to scale up, both in terms of speed and performance because of the weak pairwise metric associated to the affinity propagation algorithm \dhawat{clarify more?}.
On the other hand, the Markov clustering method can be used even in high columns or row counts while still being tractable, but give less desirable results in lower column counts.
...
Figure \ref{fig:exp_extreme}, show a sample from the errors detected by the extreme value test applied to the column \texttt{YearMade}.
It correctly identify absurd years such as negative years and future years.
As this method is based on Gaussian distributions it fails sometimes when considering a non-discreate column such as \texttt{SalePrice} having a latent variable such as \texttt{ModelId}. \dhawat{seems unclear}
This method also doesn't apply to discrete numerical columns.
To distinguish between discrete and non-discrete columns, we implemented a method that computes the ratio of the uniqueness (i.e., the number of unrepeated values to the total number of values) of the column.
If this ratio is too low, such a column is considered discrete.
For the speed and scalability of the extreme values method on nearly 900000 cells, it took a few seconds to detect the errors.
Even on a larger dataset (tested in collaboration with Foyer) the execution time was of the same order.

Finally, for the completeness test described in Section \ref{completeness} Figure \ref{fig:exp_completeness} shows some columns from rows detected as uncomplete.
...
\end{figure}
We can see from the columns shown in Figure \ref{fig:exp_completeness} that these rows doesn't cointains enough informations.
Even though the other columns of these rows are non-informative.
It is not stricto sensu an error, so these errors were not counted while computing the total number of detected errors but we believe that this type of row, less informative, is importante to be identified.
...
We tested the algorithm on a data set provided by Foyer, after comparing the flagged errors with the known errors injected by the company, it showed a success rate of 50\%, we made the choice of using very strict thresholds to avoid false positives to not flood the user with information, the major positive component is the fact that the algorithm runs relatively fast. We also proceeded to a quick test on one of the data sets of the company and the test were very encouraging as well. \
...
\end{algorithm}
\subsection{Whole row outlier detection algorithm}
...
\subsection{Text nominal logic algorithm}
...
\subsection{Mixed number text nominal logic algorithm}
