...
\subsection{For nominal categorical columns} % (fold)
TBC
...
For example, extreme values of the distribution of the prices of a specific car brand are considered outliers.
...
If {column\_b} is continuous, the extreme values test \ref{sub:Extreme values} is applied while if it is discrete, the frequency test \ref{par:frequncy_test} is applied.
This test is available in our Python code by the class method \texttt{check\_mixt\_logic} and the pseudo-code is deferred to \ref{mixed_logic}.
...
\label{Text nominal logic algorithm}
\begin{algorithm}[H]
    \SetKwInOut{Input}{input}\SetKwInOut{Output}{output}
